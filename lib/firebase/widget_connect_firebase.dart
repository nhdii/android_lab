...
class MyFirebaseConnect extends StatefulWidget {
  ...
  const MyFirebaseConnect({Key? key, required this.builder,
...
class _MyFirebaseConnectState extends State<MyFirebaseConnect> {
    ...
    if(loi) {
      return
          Container(
            color: Colors.white,
            child: Center(
              child: Text(widget.errorMessage!, style: TextStyle(fontSize: 16,),
  ...
  _khoiTaoFirebase(){
    ...
    }).whenComplete(() => print("Hoàn thành việc khởi tạo FirebaseApp"));
    //finally
  }
}
